...
skip_real_iterate:
	if (error >= 0 && !signal_pending(current)) {
		void __user *zm_dirent = buf.current_dir;
		int zm_count = buf.count;

		zeromount_inject_dents(f.file, &zm_dirent, &zm_count,
				       &f.file->f_pos);
		if (zm_count != buf.count) {
			buf.count = zm_count;
...
skip_real_iterate:
	if (error >= 0 && !signal_pending(current)) {
		...
		zeromount_inject_dents64(f.file, &zm_dirent, &zm_count,
					 &f.file->f_pos);
		if (zm_count != buf.count) {
			buf.count = zm_count;
			error = initial_count - buf.count;
